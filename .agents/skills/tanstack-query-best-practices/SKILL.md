...
| LOW      | Performance      | 5 rules | Reduces unnecessary re-renders                   |
| LOW      | Offline Support  | 2 rules | Enables offline-first patterns                   |
| CRITICAL | Architecture     | 3 rules | Enforces separation between UI and data-fetching |
...
- `qk-enum-or-factory` — Always use enums or factory functions for key values; never plain strings
...
- `mut-prefer-optimistic-over-refetch` — Prefer updating the cache in onSuccess over forced refetches; only invalidate when the server response contains data you can't reconstruct client-side
...
- `perf-id-only-list-items` — Pass only `id` to list row components; rows read their own data from the cache to prevent full-list re-renders
...
- `arch-no-tanstack-in-components` — Never import TanStack Query directly in components or pages; all query logic must live in custom hooks
- `arch-use-query-helpers` — Never use `queryClient` directly outside `tanstakQueryHelpers/`; always use the existing helpers or create a new one there
- `arch-granular-hook-files` — One TanStack Query hook per file; query hooks use the `useFetch` prefix; query key factory lives in the fetch hook file
- `no-enabled-param` — Never accept a generic `enabled: boolean` parameter; the hook derives its own enabled condition from its domain arguments
